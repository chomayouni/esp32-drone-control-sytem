...
vis_scale = 0.1;           % Scale factor for visualization (meters per pixel)
window_size = 800;         % Window size in pixels
drone_size = 10;           % Size of the drone dot in pixels
...
% Throttle input changes to produce different altitude levels:
% For 0.5s<=t<3s: low level, 3s<=t<7s: climb, t>=7s: descend
throttle_step_idx1 = round(0.5/Ts) + 1;
throttle_step_idx2 = round(3/Ts) + 1;
throttle_step_idx3 = round(7/Ts) + 1;
input_throttle(throttle_step_idx1:throttle_step_idx2-1) = 200;  % low level
input_throttle(throttle_step_idx2:throttle_step_idx3-1) = 250;  % climb to high level
input_throttle(throttle_step_idx3:end) = 150;  % descend to lower level
...
vis_fig = figure('Name', 'Drone Flight Visualization', 'Position', [100, 100, window_size, window_size]);
ax = axes('XLim', [-window_size/2*vis_scale, window_size/2*vis_scale], ...
          'YLim', [-window_size/2*vis_scale, window_size/2*vis_scale], ...
          'ZLim', [0, window_size*vis_scale]);
grid on;
xlabel('X (m)');
ylabel('Y (m)');
zlabel('Z (m)');
...
drone_marker = plot3(0, 0, 0, 'ro', 'MarkerSize', drone_size, 'MarkerFaceColor', 'r');
hold on;

% Create a line to show the drone's path
path_line = plot3(0, 0, 0, 'b-', 'LineWidth', 1.5);

% Create velocity vector visualization
vel_x_line = quiver3(0, 0, 0, 0, 0, 0, 'r', 'LineWidth', 1.5);
vel_y_line = quiver3(0, 0, 0, 0, 0, 0, 'g', 'LineWidth', 1.5);
vel_z_line = quiver3(0, 0, 0, 0, 0, 0, 'b', 'LineWidth', 1.5);

% Create text annotation for drone state
drone_info = text(0, 0, 0, '', 'FontSize', 10);

% Initialize a struct to hold the animation frames if needed
frames = struct('cdata', [], 'colormap', []);

%% Initialize drone orientation and position variables
drone_roll_angle = zeros(N, 1);
drone_pitch_angle = zeros(N, 1);
drone_yaw_angle = zeros(N, 1);
drone_x = zeros(N, 1);
drone_y = zeros(N, 1);
drone_z = zeros(N, 1);
...
for k = 2:N
    ...
    % Update drone altitude based on vertical velocity
    drone_z(k) = drone_z(k-1) + Ts * vertical_velocity(k-1);
end

%% Additional Results Visualization
figure('Name', 'Simulation Results', 'Position', [100, 100, 800, 600]);
...
% Figure for drone angles and position
figure('Name', 'Drone Orientation and Position', 'Position', [900, 100, 800, 600]);

% Plot drone angles
subplot(3, 2, 1);
plot(t, drone_roll_angle, 'r-', 'LineWidth', 1.5);
grid on;
xlabel('Time (s)');
ylabel('Roll Angle (°)');
...
subplot(3, 2, 2);
plot(t, drone_pitch_angle, 'g-', 'LineWidth', 1.5);
grid on;
xlabel('Time (s)');
ylabel('Pitch Angle (°)');
...
subplot(3, 2, 3);
plot(t, drone_yaw_angle, 'b-', 'LineWidth', 1.5);
grid on;
xlabel('Time (s)');
ylabel('Yaw Angle (°)');
...
% Plot drone position
subplot(3, 2, 4);
plot(t, drone_z, 'b-', 'LineWidth', 1.5);
grid on;
xlabel('Time (s)');
ylabel('Altitude (m)');
...
subplot(3, 2, 5);
plot(drone_x, drone_y, 'k-', 'LineWidth', 1.5);
grid on;
xlabel('X Position (m)');
ylabel('Y Position (m)');
...
% Save animation to video file if desired
% Uncomment the following lines to save the animation
if exist('frames', 'var') && ~isempty(frames)
    v = VideoWriter('drone_flight.avi');
    v.FrameRate = 20;
    open(v);
    ...
    close(v);
    disp('Animation saved to drone_flight.avi');
end
